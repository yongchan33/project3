  ...
  <%@ include file="/WEB-INF/views/member/menu.jsp" %>
	...
	<main>
      ...
      <table class="table table-striped nametable">
        <thead>
          <tr>
            <th scope="col" colspan="4"> ${requestScope.member.name } 님의 정보</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <th scope="row"><i class="far fa-user-circle"></i>      아이디 : ${requestScope.member.id }</th>
							...
							<tbody>
								<c:forEach var="rec" items="${requestScope.myproduct }">
									<tr>
										...
										<td class="cell4"><fmt:formatDate value="${rec.p_time}" type="both" dateStyle="medium" timeStyle="short" /></td>
