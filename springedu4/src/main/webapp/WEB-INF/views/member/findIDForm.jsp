...
//받아온check값과인증번호값을비교하면됨.
      ...
      //확인버튼클릭시
      function findIDBtn_f(event){
    event.preventDefault();    
    if(!chkValidation()) return
    if(certificationcheck==false){
    document.getElementById('err_certification').textContent = '"이메일인증번호가 일치하지 않습니다."'
    ...
    document.getElementById('errmsg').textContent = '"이름을입력바랍니다!"'
...
return
        ...
        function ajaxCall(event) {
          if (this.readyState == 4 && this.status == 200) {
            ...
            switch(jsonObj.rtcode){
            case "00" :
            findedIDTag.textContent = "찾으시려는아이디는"+jsonObj.result + "입니다."
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td><input type="text" name="name" id="name" placeholder="이름을입력하세요"/>
...
<div class="member_warning js_email"></div>
                   ...
                   <div class="btnli"><input class="btn btn-outline-dark insertBtn" id='findIDBtn' type="submit" value="아이디찾기" /> 
